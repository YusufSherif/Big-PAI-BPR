...
class Categorize:
                ...
                for i in outfit['items']:
                    ...
                    for word in list_of_tokens:
                        ...
                        if (not np.array_equal(zeros, vector)):
                            self.updateSet(word, vector)

            except json.decoder.JSONDecodeError:
                pass

        print("Total: ", jap2vec.exactCount)
        print("similar: ", jap2vec.simCount)
    ...
    def writeToFile(self):
        for key, value in self.cat_dict.items():
            self.cat_dict[key] = torch.tensor(value)

        torch.save(self.cat_dict, 'file.pt')
