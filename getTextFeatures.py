...
class Categorize:
    def __init__(self):
        ...
        self.wordVecs = torch.load(r'C:\Users\CSE-P07-2176-G13\Desktop\IQON3000\textDict')
        self.wordVecsKeyIndicies = list(wordVecs)
        ...
        for outfitJSON in outfitsJSON:
            try:
                ...
                for i in outfit['items']:
                    ...
                    indexList = []
                    for wordIndex in range(83):
                        index = len(self.wordVecs)
                        if(wordIndex<len(list_of_tokens)):
                            word = str(list_of_tokens[wordIndex])
                            try:
                                index = self.wordVecsKeyIndicies.index(word)
                            except ValueError:
                                index = len(self.wordVecs)
                        indexList.append(index)

                    indexTensor = torch.cuda.IntTensor(indexList)
                    self.updateSet(item_id, indexTensor)
    ...
    def writeToFile(self):
        torch.save(self.cat_set, 'textfeaturesAll')
